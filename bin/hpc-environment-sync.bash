...
function reportError() {
  ...
  # Notify syslog.
  #
  logger ${LOG2STDERR} "$(hostname) - ${SCRIPT_NAME}:${PROBLEMATIC_LINE}: FATAL: rsync of ${SOURCE} to ${DESTINATION} by ${ROLE_USER}(${REAL_USER}) FAILED!"
...
function performSync() {
  ...
  do
    ...
    do
      ...
      if [ "${LIST}" -eq 1 ]; then
        echo '================================================================================================================' >> "${RSYNC_LOG}"
        echo "  Dry run stats for syncing ${RSYNC_SOURCE} to ${RSYNC_DESTINATION}:" >> "${RSYNC_LOG}"
        echo '================================================================================================================' >> "${RSYNC_LOG}"
      fi
      set +e
      rsync ${RSYNC_OPTIONS} \
...
if [ ${ALL} -eq 1 ]; then
  ...
  #
  SOURCE="$(echo ${SOURCE} | sed "s|^${SOURCE_ROOT_PATH}/*${REFDATA_DIR_NAME}/*||")"
  #
  ...
  #
  MODULE_SPEC=(${SOURCE//\// }) || reportError ${LINENO} $?
  ...
  if [[ ${MODULE_NAME} == 'ANY' && ${MODULE_VERSION} == 'ANY' ]]; then
    ...
    #
    cd "${SOURCE_ROOT_PATH}/${MODULES_DIR_NAME}/" 2> ${TMP_LOG} || reportError ${LINENO} $?
    VERSIONED_MODULE_COUNT=$(ls -1 */${MODULE_NAME}/* | grep -c "/${MODULE_NAME}/${MODULE_VERSION}\(\.lua\)\?$") || reportError ${LINENO} $? "Cannot search for module ${MODULE_NAME}/${MODULE_VERSION} in ${SOURCE_ROOT_PATH}/${MODULES_DIR_NAME}/."
    declare -a VERSIONED_MODULES=()
    if [ ${VERSIONED_MODULE_COUNT} -ge 1 ]; then
      VERSIONED_MODULES+=$(ls -1 */${MODULE_NAME}/* | grep "/${MODULE_NAME}/${MODULE_VERSION}\(\.lua\)\?$")
...
do 
  ...
  do
    ...
    if [ -e ${DESTINATION_ROOT_DIR} ] && \
       [ -r ${DESTINATION_ROOT_DIR} ] && \
       [ -w ${DESTINATION_ROOT_DIR} ]; then
